...
// Eastern Cape City Image Mapping (Yahoo Weather Style)
const CITY_IMAGES: Record<string, string> = {
  'East London': 'https://images.unsplash.com/photo-1549405626-ec49a7442168?q=80&w=2000&auto=format&fit=crop', // Coastal vibe
  'Gqeberha': 'https://images.unsplash.com/photo-1571401138243-98282367d344?q=80&w=2000&auto=format&fit=crop', // Nelson Mandela Bay Port
  'Mthatha': 'https://images.unsplash.com/photo-1523805081730-6144a77fb30b?q=80&w=2000&auto=format&fit=crop', // Inland hills
  'Qonce': 'https://images.unsplash.com/photo-1560935574-d45607062f6b?q=80&w=2000&auto=format&fit=crop', // Historical town
  'Butterworth': 'https://images.unsplash.com/photo-1506466010722-395aa2bef877?q=80&w=2000&auto=format&fit=crop', // Rural landscape
  'Mdantsane': 'https://images.unsplash.com/photo-1516026672322-bc52d61a55d5?q=80&w=2000&auto=format&fit=crop', // Township architecture
  'Alice': 'https://images.unsplash.com/photo-1523438097201-512ae7d59c44?q=80&w=2000&auto=format&fit=crop', // Academic/Inland
  'Eastern Cape': 'https://images.unsplash.com/photo-1590483736622-39da8af7ec8d?q=80&w=2000&auto=format&fit=crop' // General EC
};
    ...
    <div className="relative min-h-screen overflow-x-hidden">
      {userRole === UserRole.BORROWER && !loggedInBorrowerId ? (
        <div className="fixed inset-0 z-[200] bg-[#1a1a1a] flex items-center justify-center p-6 overflow-hidden">
           ...
           <div className="max-w-md w-full bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative animate-in fade-in zoom-in duration-500">
              <div className="bead-accent absolute top-0 left-0 w-full opacity-10" />
              <div className="flex flex-col items-center text-center mb-10">
                <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center text-white shadow-xl rotate-3 mb-6">
                  <Lock size={32} />
                </div>
                <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight leading-none mb-4">Borrower Portal</h2>
                <p className="text-sm font-medium text-gray-400">Welcome back to your financial community hub. Please select your account to continue.</p>
              </div>

              <div className="space-y-4 mb-10">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 text-center">Available Accounts</p>
                <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                   {borrowers.map(b => (
                     <button 
                       key={b.idNumber} 
                       onClick={() => setLoggedInBorrowerId(b.idNumber)}
                       className="w-full p-5 bg-gray-50 hover:bg-indigo-50 border border-gray-100 hover:border-indigo-100 rounded-2xl flex items-center gap-4 transition-all group"
                     >
                       <div className="w-10 h-10 rounded-xl bg-white border border-gray-100 flex items-center justify-center text-indigo-600 font-black shadow-sm group-hover:shadow-md transition-all">
                          {b.name[0]}
                       </div>
                       <div className="text-left flex-1">
                          <p className="text-sm font-black text-gray-900 leading-none mb-1">{b.name}</p>
                          <p className="text-[10px] font-bold text-gray-400 font-mono">{b.idNumber}</p>
                       </div>
              ...
              <button 
                ...
                className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-indigo-600 transition-colors"
              >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 mt-6 md:mt-8">
                    ...
                    >
                      ...
                      <div className="relative z-10 flex flex-col gap-6">
                        <div className="flex items-center justify-between">
                          ...
                          <button 
                            ...
                            className="p-2.5 bg-black/30 backdrop-blur-xl rounded-2xl border border-white/20 hover:bg-black/40 transition-all text-[10px] flex items-center gap-3 font-black uppercase tracking-widest group/weather"
                          >
                            <weather.icon size={16} className={`${weather.accent} group-hover/weather:scale-110 transition-transform`} />
                      ...
                      <div className="relative z-10 space-y-6">
                        <div className="space-y-2">
                          ...
                          <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tighter leading-[0.9] drop-shadow-2xl text-white">
                            Unlock Your <br />Financial Flow
                          </h3>
                          <p className="text-white/90 text-[10px] md:text-xs font-black uppercase tracking-[0.2em]">{weather.tag}</p>
                        </div>
                        ...
                        <button onClick={() => setIsAddModalOpen(true)} className="bg-white text-emerald-950 w-full py-5 rounded-[28px] font-black text-xs md:text-sm uppercase tracking-widest shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:bg-emerald-50 active:scale-[0.98] transition-all flex items-center justify-center gap-4 group/btn overflow-hidden relative">
                           <div className="absolute inset-0 bg-emerald-400/10 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-300" />
                    ...
                    <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[48px] border border-gray-100 shadow-sm relative overflow-hidden cultural-card flex flex-col justify-between">
                      <div className="relative z-10">
                         <div className="flex items-center justify-between mb-8">
                           ...
                           <div className="text-right">
                              ...
                              <div className="px-4 py-1.5 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100">Verified</div>
                           </div>
                         </div>
                         ...
                         <div className="space-y-2 mb-8">
                           ...
                           <p className="text-[10px] text-indigo-600 font-black uppercase tracking-[0.3em]">Digital Vault 2.0 Secure</p>
                         </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 relative z-10">
                         <button onClick={() => setActiveTab('loans')} className="bg-gray-50 text-gray-900 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-gray-100 hover:bg-gray-100 transition-all flex items-center justify-center gap-2">
                           <History size={14} /> History
                         </button>
                         <button onClick={() => setSelectedBorrowerId(loggedInBorrowerId)} className="bg-[#1a1a1a] text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-black transition-all flex items-center justify-center gap-2">
                           <Eye size={14} /> Profile
                         </button>
                      </div>
                    </div>
